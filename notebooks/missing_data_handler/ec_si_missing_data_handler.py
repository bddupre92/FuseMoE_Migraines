...
def debug_patient_id_join():
    ...
    # --- 4. Reindex Patient ID Map onto Multimodal Index ---
    try:
        logging.info("Reindexing patient_id_map onto multimodal_df index...")
        # Direct reindex (like current pipeline code)
        patient_ids_reindexed = patient_id_map.reindex(multimodal_df.index)
        multimodal_df['patient_id'] = patient_ids_reindexed
        logging.info(f"Shape after reindex: {multimodal_df.shape}")
        logging.info(f"NaNs in patient_id after reindex: {multimodal_df['patient_id'].isnull().sum()}")
        reindexed_counts = multimodal_df['patient_id'].dropna().value_counts()
        logging.info(f"Patient Counts after reindex ({len(reindexed_counts)} unique non-NaN):\\n{reindexed_counts.head()}")
        # Check index again
        if multimodal_df.index.duplicated().any():
             logging.warning(f"**Duplicates found in multimodal_df index AFTER reindex! Count: {multimodal_df.index.duplicated().sum()}**")
        print("\n--- Multimodal DataFrame Head After Reindex ---")
        print(multimodal_df.head())
        print("-" * 30)

    except ValueError as ve:
         logging.error(f"*** ValueError during reindex: {ve} ***")
         logging.error("This indicates duplicate labels in one of the indexes.")
         # Check which index has duplicates right before the error
         if patient_id_map.index.duplicated().any():
              logging.error("-> patient_id_map.index has duplicates before reindex call.")
         if multimodal_df.index.duplicated().any():
              logging.error("-> multimodal_df.index has duplicates before reindex call.")
         return
    except Exception as e:
        logging.error(f"Error during reindex: {e}")
        return

    # --- 5. Apply Custom Forward Fill ---
    logging.info("Applying custom forward fill...")
    nan_indices = multimodal_df.index[multimodal_df['patient_id'].isnull()]
    fill_count = 0
    if not nan_indices.empty:
        # IMPORTANT: The fill logic needs the *original* map, not the reindexed one with NaNs
        original_id_series_for_fill = patient_id_map # Use the map before reindexing
        
        if not original_id_series_for_fill.empty:
             # Find the index of the last known ID for each NaN timestamp in multimodal_df
             # Use searchsorted on the index of known IDs from the original map
            last_known_id_indices = original_id_series_for_fill.index.searchsorted(nan_indices, side='right') - 1
            
            # Map these indices back to the actual timestamps with known IDs
            valid_indices_mask = last_known_id_indices >= 0
            fill_values_indices = original_id_series_for_fill.index[last_known_id_indices[valid_indices_mask]]
            
            # Get the patient IDs corresponding to these timestamps from the original map
            fill_values = original_id_series_for_fill.loc[fill_values_indices]
            
            # Assign the found IDs to the NaN rows in multimodal_df
            # Ensure index alignment for assignment using the nan_indices from multimodal_df
            ...
            fill_count = len(fill_series)
    logging.info(f"Filled {fill_count} NaNs using custom forward fill.")
    logging.info(f"NaNs remaining after ffill: {multimodal_df['patient_id'].isnull().sum()}")
    ffill_counts = multimodal_df['patient_id'].dropna().value_counts()
    logging.info(f"Patient Counts after ffill ({len(ffill_counts)} unique non-NaN):\\n{ffill_counts.head()}")
    print("\n--- Multimodal DataFrame Head After Forward Fill ---")
    print(multimodal_df.head())
    print("-" * 30)
    ...
    if multimodal_df['patient_id'].isnull().any():
        ...
        logging.info(f"Applying standard backfill for {missing_count} remaining NaNs...")
        multimodal_df['patient_id'] = multimodal_df['patient_id'].bfill()
        if multimodal_df['patient_id'].isnull().any():
            logging.error("Patient ID still missing after ffill and bfill! Filling with UNKNOWN_PATIENT.")
