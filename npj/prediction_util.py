...
def run_xgb(x_train, y_train, x_test):
    ...
    est = xgb.XGBClassifier(verbosity=2, scale_pos_weight = (len(y_train) - sum(y_train))/sum(y_train), seed = 42,
                            tree_method='gpu_hist', gpu_id=0, eval_metric='logloss', n_jobs=16)
